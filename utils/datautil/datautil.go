...
// SliceSubFuncs 返回切片 a 中存在但切片 b 中不存在的元素（a - b），并去除重复项。
// 根据 fna(a[i]) 和 fnb(b[i]) 返回的结果判断元素是否相等。
...
// SliceIntersectFuncs 返回切片 a 和 b 的交集（a ∩ b），并去除重复项。
// 元素的相等性由为每个切片提供的自定义函数 fna 和 fnb 决定。
func SliceIntersectFuncs[T, V any, E comparable](a []T, b []V, fna func(i T) E, fnb func(i V) E) []T {
	...
	for _, item := range a {
		// 获取切片 a 中元素的比较值
		e := fna(item)
		// 如果元素同时存在于 a 和 b 中，且尚未添加，则添加到结果中
...
// DeleteAt 删除切片元素并修改原切片，支持负数索引（从末尾计数）
...
// SliceToMapOkAny 切片转映射（自定义类型，带过滤）
...
// SliceSetAny 切片转集合（map[K]struct{}）
...
// Complete 判断切片 a 与 b 在去重后是否相等（忽略顺序）
...
// Single 返回仅存在于 a 不在 b 或仅存在于 b 不在 a 的元素集合
