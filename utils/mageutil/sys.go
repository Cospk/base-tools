...
// CheckProcessNamesExist 检查是否存在与指定路径匹配的正在运行的进程。
...
// PrintBinaryPorts 打印指定二进制进程监听的端口及其命令行参数。
...
// FindPIDsByBinaryPath 通过二进制路径查找所有匹配的进程 PID。
...
// KillExistBinary 终止所有与给定二进制文件路径匹配的进程。
